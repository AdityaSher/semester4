...
class Tree
{
  public:
    Tree() {}
    Tree_node *root = NULL;
    static Tree_node *create_node(int key)
    { //creates a node.
    ...
    static void Tree_Traversal(Tree_node *temp2)
    { //traverses a tree and print the key of a node.
        if (temp2 == NULL)
        {
            return;
        }
...
int main()
{
    ...
    if(Tree::sum_root(t2.root, find))
    {
        printf("Path for %d exists", find);
    }
    else
    {
        printf("Path for %d does not exists", find);
    }
    printf("\nNo. of possible bst for the given tree is %d", Tree::no_of_struct(Tree::count(t2.root)+1));
    return 0;
}